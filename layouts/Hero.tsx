        ...
        <Grid item xs={12} md={6}>
          <Typography lineHeight={1} color="primary" variant="h1" component="h1" mb={2} gutterBottom>
            {heroHeading.map(({text}, index) => (
              <Box key={text} sx={{overflow: "hidden", display: "inline-block"}} component="span" >
                <Box ref={(element : MutableRefObject<null>| null) => {headingRef.current[0] = element;}} sx={{display: "inline-block"}} component="span">{text} &nbsp;</Box>
              </Box>
            ))}
          </Typography>
