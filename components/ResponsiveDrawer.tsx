...
const ResponsiveDrawer = ({children, page}: Props) => {
  ...
  const drawer = (
    <div>
      <Toolbar />
      <Avatar sx={{margin: "20px auto 10px auto", width: "80px", height: "80px"}} alt="Remy Sharp" src="/static/images/avatar/2.jpg" />
      <Typography sx={{marginBottom: "20px"}}  color="black" textAlign="center">
        Remy Sharp
      </Typography>
      <Divider />
            ...
            >
              {settings.map((setting, index) => (
                <MenuItem key={setting} onClick={handleCloseUserMenu}>
                  {index === 0 ? <PersonOutlineIcon width={1} height={1} /> : <LogoutIcon width={1} height={1}  />}
                  <Typography ml={1} textAlign="center">{setting}</Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
